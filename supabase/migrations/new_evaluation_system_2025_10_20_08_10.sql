...
-- 시스템 설정 정책 (관리자만 수정, 모든 사용자 조회)
CREATE POLICY "Anyone can view system settings" ON public.system_settings_2025_10_20_08_10 FOR SELECT USING (true);
CREATE POLICY "Only admins can modify system settings" ON public.system_settings_2025_10_20_08_10 FOR ALL USING (
  EXISTS (
...
-- 주간 평가 정책 (그룹 멤버만 조회, 관리자와 팀장만 수정)
...
CREATE POLICY "Admins and leaders can manage evaluations" ON public.weekly_evaluations_2025_10_20_08_10 FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.user_profiles_2025_09_27_12_14 up
    WHERE up.user_id = auth.uid() AND (
      ...
      EXISTS (
        ...
        WHERE sg.id = weekly_evaluations_2025_10_20_08_10.group_id AND sg.leader_id = up.id
      )
    )
  )
);

-- 개인 점수 정책 (본인과 같은 그룹 멤버만 조회)
CREATE POLICY "Members can view group scores" ON public.member_weekly_scores_2025_10_20_08_10 FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Group members can manage role opinions" ON public.role_opinions_2025_10_20_08_10 FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.weekly_evaluations_2025_10_20_08_10 we
    JOIN public.group_memberships_2025_09_27_12_14 gm ON gm.group_id = we.group_id
    JOIN public.user_profiles_2025_09_27_12_14 up ON up.id = gm.user_id
...
CREATE POLICY "Admins and leaders can manage team settings" ON public.team_settings_2025_10_20_08_10 FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.user_profiles_2025_09_27_12_14 up
    WHERE up.user_id = auth.uid() AND (
      up.role = 'admin' OR 
      EXISTS (
        SELECT 1 FROM public.study_groups_2025_09_27_12_14 sg
        WHERE sg.id = team_settings_2025_10_20_08_10.group_id AND sg.leader_id = up.id
      )
    )
  )
);
